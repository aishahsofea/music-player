...
const Playlist = (_ => {
    // data or state
    let songs = songsList;
    let currentlyPlayedIndex = 0;
    let currentSong = new Audio(songs[currentlyPlayedIndex].url);
    let isPlaying = false;
    ...
    const changeAudioSrc = _ => {
        currentSong.src = songs[currentlyPlayedIndex].url;
    }
    ...
    const mainPlay = clickedIndex => {
        if (currentlyPlayedIndex === clickedIndex) {
            togglePlayPause();
        } else {
            currentlyPlayedIndex = clickedIndex;
            changeAudioSrc();
            togglePlayPause();
        }
    ...
    const playNext = _ => {
        if (songs[currentlyPlayedIndex + 1]) {
    ...
    const render = _ => {
        ...
        const toggleIcon = itemIndex => {
            if (currentlyPlayedIndex === itemIndex) {
        ...
        songs.forEach((songObj, index) => {
            markup += `
            <li class="playlist__song ${index === currentlyPlayedIndex ? "playlist__song--active" : ""}">
